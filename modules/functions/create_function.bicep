...
resource r_fnAppSettings 'Microsoft.Web/sites/config@2021-03-01' = {
  ...
  properties: {
    ...
    WAREHOUSE_STORAGE: 'DefaultEndpointsProtocol=https;AccountName=${saName1};EndpointSuffix=${environment().suffixes.storage};AccountKey=${r_sa_1.listKeys().keys[0].value}'
...
resource r_fn_1 'Microsoft.Web/sites/functions@2022-03-01' = {
  ...
  properties: {
    config: {
      ...
      //     path: '${blobContainerName_1}/{name}.json' // The path to the blob that triggered the function,
    ...
    files: {
      'function.json': replace(loadTextContent('../../app/function_code/hello_world/function.json'),'BLOB_CONTAINER_NAME', blobContainerName_1)
